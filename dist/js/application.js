...
// Responsive
var mqBreakpoints = [['sm', 767], ['md', 768], ['lg', 1025]]; // Not responsive
...
if (enable.jQueryUI.datepicker === true) {
  ...
  $.datepicker.setDefaults($.datepicker.regional.ru); // Datepicker
...
if (enable.components.fonts === true) {
  if (enable.components.fontsRubleSans === true) {
    ...
    fontALSRublArial.load().then(function () {// console.log('ALSRubl-Arial has loaded.');
    });
  }

  if (enable.components.fontsRubleSerif === true) {
    var fontALSRublTimes = new FontFaceObserver('ALSRubl-Times');
    fontALSRublTimes.load().then(function () {// console.log('ALSRubl-Times has loaded.');
    });
  }
}
"use strict";
